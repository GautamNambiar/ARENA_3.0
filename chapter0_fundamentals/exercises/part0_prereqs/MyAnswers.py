...
def gather_2d(matrix: Tensor, indexes: Tensor) -> Tensor:
    ...
    assert indexes.size(0) <= matrix.size(0), "index size exceeds matrix size"
    assert ((indexes >= 0) & (indexes < matrix.size(1))).all(), "index values out of bounds"
...
def batched_logsumexp(matrix: Tensor) -> Tensor:
    ...
    matrix_rel = matrix - max_vals
    return matrix_rel.exp().sum(1).log() + max_vals.squeeze(1)
    raise NotImplementedError()
...
# %%
# Exercise H1 (batched logsumexp) (Einops solution)
def batched_logsumexp(matrix: Tensor) -> Tensor:
    """For each row of the matrix, compute log(sum(exp(row))) in a numerically stable way.
    ...
    matrix_rel = matrix - einops.rearrange(max_vals, "n -> n 1")
    return matrix_rel.exp().sum(1).log() + max_vals
    raise NotImplementedError()
...
def batched_softmax(matrix: Tensor) -> Tensor:
    ...
    """
    matrix_max = matrix.max(dim=1).values
    matrix_rel = matrix - einops.rearrange(matrix_max, "n -> n 1")
    return matrix_rel.exp()/matrix_rel.exp().sum(dim=1, keepdim=True)
    raise NotImplementedError()
...
def batched_logsoftmax(matrix: Tensor) -> Tensor:
    ...
    matrix_max = matrix.max(dim=1).values
    matrix_rel = matrix - einops.rearrange(matrix_max, "n -> n 1")
    return matrix_rel - matrix_rel.exp().sum(dim=1, keepdim=True).log()
    raise NotImplementedError()
...
def batched_cross_entropy_loss(logits: Tensor, true_labels: Tensor) -> Tensor:
    ...
    cross_ent_local = -logprobs.gather(1, indices)
    return einops.rearrange(cross_ent_local, "n 1 -> n")
...
assert_all_equal(actual, expected)
# %%
A=t.Tensor([[1,2,3,4],[2,3,4,5],[1,5,6,7],[2,3,4,5]])
ind = [0,1,1]
print(A[[[0],[3]]])
# %%
...
def einsum_mv(mat: np.ndarray, vec: np.ndarray):
    ...
    """
    return einops.einsum(mat, vec, "a b, b -> a")
    raise NotImplementedError()
